...
The cleaned rule index represents each Index Diachronica rule line as one index rule whose change spine is an ordered **`stages`** list of opaque Index-shaped strings (length 2 = single step; length ≥ 3 = chain; `stages: []` + `status: skipped` = hold-out). YAML no longer carries `input`/`output`. Parse isolates at most one `env` and one `exception` on the rule, then splits the spine on every `→`. Compile still expands only at compile time into adjacent ASCA pairs, stamping that same rule-level `env`/`exception` on each emitted step. This supersedes storing chains as `" > "` inside a string `output` and the earlier rejection of a structured spine in favour of opaque `input`/`output` strings (ADR-0005 amendment / no-ingest-split ticket 01), while keeping one index rule per HTML line.
