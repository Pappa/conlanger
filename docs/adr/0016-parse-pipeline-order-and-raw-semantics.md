...
2. **Manual mapping** (`manual_mappings.yml`) — longest `from` first; substring or regex rewrite on the working copy.
3. **Index rule normalisation** — ordered, policy-documented surface transforms on the working line ([ticket 143](../../.scratch/rule-index/issues/143-implement-index-rule-normalisation-passes.md); placeholder no-op until passes land).
...
- Parser doc Phase A½ table and `IndexDiachronicaParser.parse_rule_element` must match this order (manual mappings before index rule normalisation before section mappings).
