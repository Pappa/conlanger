...
Maintainers can replace individual Index rule lines without editing `index_diachronica_original.html`. Overrides live in `data/diachronica/index_diachronica_corrections.yml` as a flat map from HTML **rule id** (`p.schg` `id` attribute) to a Unicode Index rule string (no `<sub>` markup).
