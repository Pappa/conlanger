...
**Intended** here is CONTEXT + the ADRs, with later ADRs winning when they conflict with the stage docs. That matters: `docs/index-diachronica-parser.md` still describes `input`/`output` and parse-time chain split; [ADR-0011](docs/adr/0011-corpus-rule-stages.md) and [ADR-0005](docs/adr/0005-one-corpus-rule-per-html-line.md) superseded that. ⚠️ marks where **code** does not match that intended design (not where a stage doc is merely stale).

Placement in the four-stage pipeline ([SYSTEM.md](docs/SYSTEM.md)):

- Index Diachronica parse
- Applier-neutral corpus validation (inventory lives here; it *calls* compile + compile validation per rule)
...
- **Index Diachronica parse** (HTML → applier-neutral **rule corpus** YAML; [ADR-0002](docs/adr/0002-applier-neutral-yaml-rule-corpus.md), [ADR-0006](docs/adr/0006-html-source-of-truth-yaml-successor.md))
  - **Ingest**
  ...
  - **Processing**
    ...
    - Each `<p class="schg">` → one **corpus rule** (1 HTML line → 1 YAML row; [ADR-0005](docs/adr/0005-one-corpus-rule-per-html-line.md))
    ...
    - Write `data/diachronica/index_diachronica_parsed.yml` (`uv run regenerate_corpus`)
  - **Transformation** (per rule; corpus fields stay Index-shaped / applier-neutral — not ASCA syntax)
    ...
    - Strip leading `— `; isolate one `env` and one `exception`; split the change spine on **every** `→` into **stages** ([ADR-0011](docs/adr/0011-corpus-rule-states.md))
    - Remaining arrows inside a field → `>` (ASCA chain glyph in an opaque string, not a second corpus row)
    ...
    - One corpus rule per line: **no** parse-time explosion of chains into extra YAML rows
...
- **Applier-neutral corpus validation** (inventory)
  - Sits here in the pipeline: after parse, it compiles and compile-validates each corpus rule to produce the inventory CSVs. Not detailed further.
- **Applier compile** (corpus section → ASCA `.rsca` string; corpus YAML is never mutated)
  - **Ingest**
  ...
  - **Processing** (`DiachronicSeries` then `SoundChangeRule`)
    ...
    - For each **corpus rule**, compile expands `stages` into adjacent pairs at **compile time** only ([ADR-0011](docs/adr/0011-corpus-rule-stages.md)): `[A, B, C]` → two steps `A > B` and `B > C`, each stamped with the same rule-level `env` / `exception`
    - Each step is one `SoundChangeRule` (requires `input` / `output` *after* that expansion — those are compile-time fields, not corpus YAML)
...
- **Index Diachronica parse** (`IndexDiachronicaParser` in `src/conlanger/tools/ingest/parser.py`; orchestrated by `regenerate_corpus`)
  - **Ingest**
  ...
  - **Processing**
    ...
    - `abbreviations()` → `{}`
    - Write YAML via `write_cleaned_corpus`; then inventory unless `--skip-validation`
...
- **Inventory**
  - After YAML write: each corpus rule → mini-section → `DiachronicSeries` → `validate_asca` → CSV. Same compile path as below.
- **Applier compile** (`src/conlanger/tools/rules.py` + `src/conlanger/tools/compile/asca/pipeline.py`)
  ...
  - **Processing — `DiachronicSeries`**
    ...
    - For each corpus rule:
      - `normalize_corpus_rule_tilde_fields(rule)` (mutates a copy of `stages`; a `" > "` inside a stage can **split into extra stages**)
      - `expand_chained_corpus_rule` reads **only** `stages`; emits `N−1` dicts `{input, output, …meta}` for adjacent pairs; copies `env` / `exception` / `comment` / `sporadic` / `skip` onto every step; `< 2` non-empty stages → `[]` (parse `status: skipped` rules simply **vanish** from the `.rsca`)
