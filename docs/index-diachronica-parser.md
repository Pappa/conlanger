...
| Output / env / exception split | implemented | C3 | ASCA env uses `/`; exception via `!`, word `except`, or second `/`. Residual Index lines that omit `/` before a space-separated env (`h #_`, `∅ VC_CV`) or that put an env-shaped last chain stage after `→` (`∅ → dz → î_V`) peel that suffix into `env` ([pass 82](../.scratch/cleaned-rule-corpus/issues/82-correction-pass-output-env-slash-boundary.md)). | `split_post_arrow`, `split_env_exception`, `peel_glued_env_from_output` |
