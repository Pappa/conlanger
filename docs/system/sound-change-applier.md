...
| Index identity exceptions (`! Host = seg`) | implemented | 7½ | `resolve_index_identity_exceptions`: Family A env-slot splice → `compiled_exception`; Family B input narrowing (`-seg`) with `compiled_exception = None`. Deferred Family B after host-bracket normalization when input uses Index bracket matrices (`V[-long]`). | **After group mappings, before subscripts** — RHS class letters must expand first; Family B bracket deferral runs again after `normalize_asca_host_bracket_matrices`. |
...
| Bare env/exception feature matrices | implemented | 7¾ | `resolve_bare_env_exception_feature_matrices`: bare `env: '[±F]'` → segment-at-focus matrix on **input**, `env = _`; bare `exception: '[±F]'` → inverse polarity on **input**, `compiled_exception = None`. | **After post-subscript on I/O, before I/O `normalize_asca_host_bracket_matrices`** — bracket merge on input, then colon normalization. |
| I/O host+bracket matrices → colon | implemented | 8 | `normalize_asca_host_bracket_matrices` on compiled input/output (ticket 121). | After bare env/exception cross-field step when input carries merged matrices. |
| Env/exception host+bracket matrices → colon | implemented | 10½ | `normalize_asca_host_bracket_matrices` on compiled env/exception after post-subscript (ticket 131 Family C). | **After env/exception post-subscript, before set wrapping** — neighbour-host postfix (`C[+voice]_` → `C:[+voice]_`); standalone neighbour templates (`_[+rtr], [+rtr]_`) pass through. |
| Env/exception merge adjacent / colon-chained matrices | implemented | 10¾ | `normalize_asca_adjacent_feature_matrices` on compiled env/exception after host-bracket colon pass (tickets 124, 133). | **After env/exception `normalize_asca_host_bracket_matrices`, before set wrapping** — merges `][` and `:[a]:[b]` on one host (e.g. class expansion + stress). |
