...
**Primary code:** `[src/conlanger/tools/ingest/parser.py](../src/conlanger/tools/ingest/parser.py)` (`IndexDiachronicaParser(parser_config)`); parse-time rule rows are built as pydantic `IndexRule` / `IndexContext` (`[index_models.py](../src/conlanger/tools/ingest/index_models.py)`, [ADR-0017](./adr/0017-structured-env-exception-indexcontext.md)). **Bracket-aware scanning** (nested `{…}`, optional `(…)`, matrices `[…]`, comma/space splits): `[src/conlanger/utils/bracket_scanner.py](../src/conlanger/utils/bracket_scanner.py)` (`BracketScanner`, `split_outside_brackets`). **Nested set flatten** (env / exception / stages): `[flatten_nested_sets.py](../src/conlanger/tools/ingest/flatten_nested_sets.py)`. **Collective subscript expansion:** `[src/conlanger/utils/series.py](../src/conlanger/utils/series.py)` (`apply_series_expansions` from `ParserConfig.series_expansions`). **Correspondence-series indices** stay Index-shaped at parse and expand at [compile](./sound-change-applier.md) via `CompilerConfig`. **Config I/O:** `[config_loaders.py](../src/conlanger/scripts/config_loaders.py)` reads `config/parser/`; **orchestration:** `uv run create_index` → `[create_index.py](../src/conlanger/scripts/create_index.py)`.
...
| Index rule normalisation                          | placeholder | A½2   | Ordered surface transforms after manual mappings; passes in [ticket 143](../.scratch/rule-index/issues/143-implement-index-rule-normalisation-passes.md) ([ADR-0016](./adr/0016-parse-pipeline-order-and-raw-semantics.md)).                               | `apply_index_rule_normalisation` |
...
| Output / env / exception split           | implemented | C3    | ASCA env uses `/`; exception via `!`, word `except`, or second `/`. A slash glued to output (`∅/ _#`) is the same delimiter without the preceding space ([pass 82](../.scratch/rule-index/issues/82-correction-pass-output-env-slash-boundary.md)); symmetrically, env glued after a spaced slash (`∅ /{a,E}_`) splits when the character after `/` is env-start ([pass 132](../.scratch/rule-index/issues/132-correction-pass-env-slash-glued-and-sporadic-question-mark.md)). Lines that omit `/` entirely are Index errata — overlay in `index_diachronica_corrections.yml`, not a parse peel. | `split_post_arrow`, `split_env_exception`                |
...
| Step                                                                  | Status      | Order | Rationale                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | Code                       |
| --------------------------------------------------------------------- | ----------- | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------- |
| Uncertainty gloss → `sporadic: true` + `comment`                      | implemented | D1    | `sporadic`/`sometimes`/`occasionally` are editorial qualifiers, not ASCA syntax ([pass 19](../.scratch/rule-index/issues/19-correction-pass-sporadic-qualifier.md)); a trailing `?` / `?` on env or exception is the same uncertainty marker ([pass 132](../.scratch/rule-index/issues/132-correction-pass-env-slash-glued-and-sporadic-question-mark.md)). Stripped prose captured, not discarded. Runs on remainder only (not **rule comment** seeded at B½).           | `apply_sporadic_qualifier` |
...
| Nested `{…}` set flatten (env / exception / stages)     | implemented | F1½   | ASCA rejects same-type nested braces; union + parenthetical-in-set rewrite via `BracketScanner` ([passes 69–70](../.scratch/rule-index/map.md)). `raw` unchanged. Runs immediately after F1.                                                                                              | `flatten_nested_sets_in_section_rules`      |
