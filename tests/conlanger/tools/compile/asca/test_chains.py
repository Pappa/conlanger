...
def test_expand_chained_index_rule_splits_stages_chain():
    assert expand_chained_index_rule({"stages": ["dʒ", "tʃ", "ʃ"]}) == [
        {"input": "dʒ", "output": "tʃ"},
        {"input": "tʃ", "output": "ʃ"},
    ]


def test_expand_chained_index_rule_keeps_single_step():
    rule = {"stages": ["a", "e"], "env": "_#"}
    assert expand_chained_index_rule(rule) == [
        {"input": "a", "output": "e", "env": "_#"}
    ]


def test_expand_chained_index_rule_empty_stages_emits_nothing():
    assert expand_chained_index_rule({"stages": []}) == []


def test_expand_chained_index_rule_single_stage_emits_empty_output():
    assert expand_chained_index_rule({"stages": ["a"]}) == [
        {"input": "a", "output": ""}
    ]


def test_expand_chained_index_rule_propagates_env_to_each_step():
...
def test_expand_chained_index_rule_propagates_exception_and_comment():
    rule = {
        "stages": ["a", "b", "c"],
        "env": "_V",
        "exception": "C_",
    ...
    steps = expand_chained_index_rule(rule)
    assert steps == [
        {
            "input": "a",
            "output": "b",
...
def test_expand_chained_index_rule_propagates_sporadic_to_each_step():
...
def test_sound_change_ruleset_expands_chained_index_rule():
    section = {
        "index": "1.0",
        "section": "Chain",
