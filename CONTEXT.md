...
**IndexContext**:
Structured object for rule-level **`env`** or **`exception`** in YAML and at parse time on **`IndexRule`** ([ADR-0017](docs/adr/0017-structured-env-exception-indexcontext.md)). Optional keys: `context` (Index-shaped target/structural string), `position` (open map of relation names to `bool` | `str` | `list[str]`), `dialect` (`bool` | `str` | `list[str]`). Inclusive vs exclusive scope follows field placement (`env` vs `exception`), not extra flags on the object. Compile projects to env/exception strings ([ticket 144](.scratch/rule-index/issues/144-implement-indexcontext-compile-resolution.md)).
_Avoid_: top-level `position: {env?, exception?}` from superseded ticket 122; rule-level `applicability` for dialect (use `dialect` on `IndexContext` only)

**Environment**:
The phonological context in which a sound change applies — the `/ … _` portion of a rule (where the change is conditioned). Stored as optional field `env` on a index rule; absent means any environment. Value is an opaque Index-shaped **string** or an **`IndexContext`** object ([ADR-0017](docs/adr/0017-structured-env-exception-indexcontext.md)); string form remains valid until corpus regen. Index prose catch-all **`else`** is not itself an environment — after parse resolution it becomes an **exception** derived from the previous rule (see correction pass on `/ else`). Index prose **`medial`** / **`medially`** means word-internal (not word-initial, not word-final) — after parse resolution bare forms become `_` with a boundary **exception** (ASCA `// :{#_, _#}:`); tentative qualifiers in **`comment`** do not narrow the rewrite. Index **position** env prose (e.g. **`final syllables`**, **`syllable-final`**, **`next to {X}`**, **`unstressed syllables`**, trailing ``, in monosyllables`` on a structural env) normalizes at parse time to ASCA-shaped `env` values (`U#`, `_,{…}`, `_ %[-stress]`, …); removed prose is captured in **`comment`**; rules that already have a separate **`exception`** keep bare env rewrites deferred (qualifier stripping still applies). Bare Index **`/ unstressed`** and **`/ stressed`** (no “syllable(s)”) are **segment-level**: parse maps them to standalone `[-stress]` / `[+stress]` in `env`, meaning the **changed segment at `_`** must be unstressed or stressed — distinct from syllable-level **`unstressed syllables`** (`_ %[-stress]`). A standalone **feature matrix** in `env` with no `_` and no neighbour host qualifies the **changed segment at `_`** (segment-at-focus); a matrix postfix on a class letter or segment host in a structural template (e.g. `C[+voice]_`, `V[-long]_#`) qualifies **that neighbour host**. `env` keeps Index postfix brackets on hosts; colon-on-underscore (`_:[+feature]`) and ASCA colon syntax are **invalid SoT**. At compile, segment-at-focus bare matrices move to **input** with colon; neighbour host matrices become bracket→colon on that host **in env** (not on input).
_Avoid_: “context” when `exception` is meant; prose paragraphs from Index comments; treating bare `else` as a valid `env` value in the cleaned SoT; equating medial with intervocalic (`V_V`) or with boundary shorthand `#_, _#`; treating bare `/ unstressed` as syllable-level; `_:[+feature]` placeholders in the cleaned SoT; parse-time rewrite of env into ASCA colon form

**Exception**:
A phonological context that blocks an otherwise applicable change — the `! …` or `| …` portion of a rule. Stored as optional field `exception` on a index rule; absent means no exceptions. Value is an opaque Index-shaped **string** or an **`IndexContext`** object ([ADR-0017](docs/adr/0017-structured-env-exception-indexcontext.md)); string form remains valid until corpus regen. For Index `/ else` rules whose previous sibling has an environment and no exception, parse writes that previous environment into `exception` and omits `env` (complementary default branch). For Index word-internal **`medial`** / **`medially`** env prose (when no separate exception is already present), parse sets `exception: :{#_, _#}:` to block word-initial and word-final positions. A standalone **feature matrix** in `exception` (no neighbour host, no identity `= segment`) blocks when the **changed segment at `_`** carries that feature — the complement of the same matrix in `env`; at compile, polarity flips and the constraint narrows **input** (not a bare `// [±feature]` exception). A matrix postfix on a neighbour host in an exception template qualifies **that host**; at compile, bracket→colon on the host in the exception field. Index identity **`! Host = segment`** is a separate compile family ([ticket 128](.scratch/rule-index/issues/128-correction-pass-index-identity-exceptions.md)).
_Avoid_: the English word “except” in citation prose; conflating with environment; leaving Index `else` in `env`; treating a bare exception matrix as a neighbour-segment or syllable-level predicate by default; parse-time rewrite of exception into ASCA colon form
...
_Avoid_: treating raw as byte-identical to the on-disk HTML; treating `stages`/`env`/`exception` as the only recoverable form of the Index line; storing correction overlay text in `raw`

**Index Diachronica correction**:
A maintainer-authored replacement for one Index rule line, keyed by **rule id**, stored as a Unicode Index string (not HTML). When present, it replaces the extracted line on the parse **working copy** before manual mapping; **`raw`** and **`source`** still refer to the HTML line ([ADR-0016](docs/adr/0016-parse-pipeline-order-and-raw-semantics.md)).
...
**Manual mapping**:
A maintainer-authored rewrite of part or all of an Index rule string, keyed by a `from` pattern. At parse, after the correction overlay on the working line, it replaces matches of `from` with `to` on that working copy (literal substring, or a regular expression when flagged); **`raw`** is unchanged. Hits are logged to `manual_mappings_matched_rules.csv` at regen.
...
**Feature matrix**:
A distinctive-feature bundle in square brackets, attached to a segment or class letter (e.g. `V[+long]`, `C[+voice]`, `[+voice]` alone) — the standard handbook / Index Diachronica convention: the bracketed features apply to the immediately preceding segment or class. **Stages**, **`env`**, and **`exception`** keep this postfix bracket form; it is applier-neutral. At HTML→YAML ingest, feature *names* inside `[...]` are normalised via `config/parser/feature_mappings.yml` (1:1 renames, bundles, polarity inverts); bracket syntax is unchanged. At ASCA **compile**, host+bracket matrices on input/output compile fields are rewritten to colon form (`V[+long]` → `V:[+long]`) because ASCA treats postfix brackets as separate tokens, not as a matrix on the host. In **`env`** / **`exception`**: a standalone matrix with no neighbour host qualifies the **changed segment at `_`** (compile moves segment-at-focus constraints to **input** with colon, or flips polarity for bare exception matrices); a matrix postfix on a host in a structural template qualifies **that neighbour** (compile = bracket→colon on the host in the same field). Multi-token neighbour templates (e.g. `_ [+rtr], [+rtr]_`) stay Index-shaped in SoT. `raw` preserves the original HTML form.
_Avoid_: listing individual ASCA feature names or shorthands in this glossary; “features” when meaning phoneme-inventory dimensions; storing colon matrices in the rule index YAML; assuming postfix brackets and colon matrices are interchangeable in ASCA; `_:[+feature]` as index notation; parse-time rewrite of env/exception matrices into ASCA form
