...
**Parallel columns**:
Index notation where one **compile field** lists several top-level segments separated by spaces outside `{…}`, `(…)`, and `[…]`, each segment aligned with the corresponding segment on the other side of the change (e.g. `c ɲ > ∅ n`; set columns `{r,h} > {∅,h}`). Project tickets sometimes call this “condensed”; that word is not a glossary term here.
_Avoid_: conflating parallel columns with comma-separated **set** members inside `{…}`; treating every space-separated token list as parallel columns when only one side is involved
