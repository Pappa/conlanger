...
def _expand_set_cross_optional(left: str, right: str, suffix: str) -> str:
    left_members = _split_set_members(left)
    right_members = _split_set_members(right)
    expanded: list[str] = []
    seen: set[str] = set()
...
def _expand_set_suffix_optionals(text: str) -> str:
    def repl(match: re.Match[str]) -> str:
        ...
        if re.fullmatch(r"[A-Z]", inner):
            members = _split_set_members(left)
            expanded: list[str] = []
...
def _expand_literal_set_optionals(text: str) -> str:
    def repl(match: re.Match[str]) -> str:
        ...
        members = _split_set_members(match.group(2))
...
def _expand_prefix_structure_optional(token: str) -> str:
    ...
    if braced:
        ...
        if set_match:
            members = _split_set_members(set_match.group(1))
