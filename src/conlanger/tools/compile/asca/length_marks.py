...
- ``segmentː`` / ``segment(ː)`` → ``segment:[+long]``
- ``{set}ː`` / ``{set}(ː)`` / bare ``ː`` set members → set or ``V:[+long]`` member
- ``Matrix]ː`` / ``Matrix](ː)`` → ``Matrix, +long]`` (matrix-suffix length)
- ``Vn(ː)`` template optional length → ``Vn:[+long]`` (Salish positional vowels)
...
_TEMPLATE_OPT_LENGTH_RE = re.compile(rf"([A-Z]\d+)\({_LENGTH}\)")
...
_POST_MATRIX_OPT_LENGTH_RE = re.compile(r"\]\(" + re.escape(_LENGTH) + r"\)")
...
def normalize_asca_length_marks(text: str) -> str:
    ...
    text = _TEMPLATE_OPT_LENGTH_RE.sub(r"\1:[+long]", text)
    text = _POST_MATRIX_OPT_LENGTH_RE.sub(", +long]", text)
