...
_SEGMENT_HALF_LENGTH_RE = re.compile(rf"({_IPA_SEGMENT}){re.escape(_HALF_LENGTH)}(?!:)")
...
def normalize_asca_length_marks(text: str) -> str:
    ...
    if not text or (_LENGTH not in text and _HALF_LENGTH not in text):
        return text
    text = _GROUPING_LENGTH_RE.sub(r"\1:[+long]", text)
    text = _REF_LENGTH_RE.sub(r"\1:[+long]", text)
    text = _SEGMENT_HALF_LENGTH_RE.sub(r"\1:[+long]", text)
