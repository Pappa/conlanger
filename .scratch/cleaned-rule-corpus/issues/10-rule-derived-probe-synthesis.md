...
- **Input surface:** **post-mapping** compiled strings — the same `input` / `output` / `env` / `exception` fields that `DiachronicSeries` emits after group-letter expansion (not Index-shaped `raw`).
