...
- Glossary: `CONTEXT.md` — **Subscript notation** (four uses); **Stages**; **Optional outputs** (grill 61 resolved). Decisions: ADRs 0001–0011.
...
- **Correction priority (2026-08-12):** [62 tone](issues/62-correction-pass-tone-features.md) → [63 near-miss unknown_character](issues/63-correction-pass-near-miss-unknown-character.md) → re-review → [64 syntax_other spike](issues/64-spike-syntax-other-near-miss-sections.md) (backlog). Parallel when free: ~~[65 series coverage](issues/65-series-mappings-coverage-pass.md)~~ done — **policy revisit:** [grill 72](issues/72-grill-series-mapping-manual-sot.md) ingest + drop parse CSV; [grill 73](issues/73-grill-series-mapping-config-sot.md) series config SoT. Optional outputs: [66](issues/66-implement-optional-outputs-alt-idx.md) resolved; [67 nested sets spike](issues/67-spike-nested-sets.md) resolved → [69](issues/69-correction-pass-flatten-nested-context-sets.md) / [70](issues/70-correction-pass-flatten-nested-io-sets.md) / [71 grill](issues/71-grill-paren-and-parallel-set-notation.md); [68 sporadic](issues/68-sporadic-sampling.md) `needs-triage`.
- **`data/` folder:** project data tree — use for reference **and** implementation. Index Diachronica HTML SoT: `data/diachronica/index_diachronica_original.html`; regenerated corpus YAML: `data/diachronica/index_diachronica_parsed.yml`; runtime ASCA mappings under `data/asca/` — **`group_mappings.csv`** (Index **class letters** at compile); **`asca_aliases.alias`** (word (de)romanisation via `asca -l` only — not rule grouping letters); **`series_mappings.csv`** (correspondence-series / collective at parse).
...
- [Full-corpus validation inventory](issues/12-full-corpus-validation-inventory.md) — `uv run regenerate_corpus`; per-rule CSV + summary under [inventory/](inventory/). Current baseline: **7447 / 9201 ok (80.9%)**; **247 / 714 sections all OK (34.6%)** on ASCA 0.10.2.
...
- [Spike: nested sets](issues/67-spike-nested-sets.md) — **resolved** (2026-08-12): 46 `nested_brackets` rows bucketed; 0 optional-output overlap. Follow-ups: [69](issues/69-correction-pass-flatten-nested-context-sets.md) env/exception flatten, [70](issues/70-correction-pass-flatten-nested-io-sets.md) I/O flatten (blocked by [71](issues/71-grill-paren-and-parallel-set-notation.md)), defer prose/malformed per-rule. Findings: [research/nested-sets-inventory.md](research/nested-sets-inventory.md).
...
3. **[Correction passes](issues/13-correction-pass-template.md)** — in progress. **Next (section-completeness):** [62 tone → `[tone: N]`](issues/62-correction-pass-tone-features.md) → [63 near-miss unknown_character](issues/63-correction-pass-near-miss-unknown-character.md) → re-review → [64 syntax_other near-miss spike](issues/64-spike-syntax-other-near-miss-sections.md) (backlog). ~~[28 extract series](issues/28-extract-correspondence-series-mappings-from-html.md)~~ / ~~[27 expand](issues/27-implement-parse-time-correspondence-series-expansion.md)~~ done → [65 series coverage A–E](issues/65-series-mappings-coverage-pass.md). ~~Feature Phase 1/2 place bundles~~ done; tone pass is 62.
...
- **Subscript notation (all four uses)** — grilled + researched: [subscript-notation-index-asca-brassica.md](research/subscript-notation-index-asca-brassica.md), [positional-slots-and-identity-subscripts.md](research/positional-slots-and-identity-subscripts.md). **Working policy:** correspondence-series + collective → **parse-time**; positional + identity → **compile-time** (Index-shaped in YAML). **Phase 1:** [Correction pass: positional slots and identity subscripts](issues/40-correction-pass-positional-identity-subscripts.md) (+51 ok). **Phase 2:** ~~[Correction pass: subscript edge cases](issues/41-correction-pass-subscript-edge-cases.md)~~ done (+14 ok; matrix/ˤ/compounds/optional shipped; prose + `Bʱ₁` hold-outs). **Compile order:** [research/asca-compile-transform-order.md](research/asca-compile-transform-order.md). **Open:** [Grill 72 ingest pipeline + remove parse series CSV](issues/72-grill-series-mapping-manual-sot.md); series ontology deferred to [Grill 73: series expansion config](issues/73-grill-series-mapping-config-sot.md).
