...
4. **Validation confidence (2026-08 charted):** implement [Inventory success/error CSV splits and ok-change changelog](issues/34-inventory-success-error-splits-and-ok-changelog.md) **first**; then [Field-isolation inventory sidecar](issues/36-field-isolation-inventory-sidecar.md) (spike done). Open cluster work remains available: [Debug / fix: Index `same POA`](issues/33-debug-same-poa-feature-artefact.md).
