...
Target cluster: `syntax_other` — **`Expected end of line… Did you forget a '/' between the output and environment?`** — **23** rules in ≤3-fail sections (**9** mono-class sections would complete if cleared). Full-index count at claim: **68** ([inventory summary](../inventory/rule-inventory-summary.md)).
