...
| File | Rows |
|------|------|
| `field-isolation.csv` | **Default: fails-only** (`whole_ok == false`); optional `--field-isolation-all` writes all rows |
| `field-isolation-success.csv` | `whole_ok == true` (no errors) |
| `field-isolation-error.csv` | `whole_ok == false` (errors) |

Success and error splits use the **same columns** as the full field-blame CSV and are **rewritten every regen** (mirror [34](34-inventory-success-error-splits-and-ok-changelog.md)). When the main CSV is fails-only, its rows are a subset of `field-isolation-error.csv`.
...
- Build the full field-blame dataframe, then:
  - write `field-isolation-success.csv` / `field-isolation-error.csv` (filter on `whole_ok`)
  - write `field-isolation.csv` — **default: fails-only** (`whole_ok == false`); optional `--field-isolation-all` for all rows
...
- 2026-08-20: Add `field-isolation-success.csv` / `field-isolation-error.csv` filtered views (`whole_ok` split).
...
- [x] Summary links `field-isolation.csv`, `-success.csv`, and `-error.csv`
