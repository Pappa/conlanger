...
- **Structural split first:** `extract_rule_parts` runs on the working line **before** comment capture. Output **chain split** uses every `→` in the post-arrow segment — arrows inside editorial tails can create spurious `stages` (e.g. [Archi `Archi-ɢ,ɣ`](inventory/rule-inventory-error.csv) / manual mapping `ɣ → q ;; (more likely, *ɢ → q instead of → ɣ)`).
