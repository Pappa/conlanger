...
Target cluster: `unknown_character` — residual typographic quotes **`"`** **`"`** **`'`** (U+201C/U+201D/U+2019) — **57** rules at current inventory baseline ([summary](../inventory/rule-inventory-summary.md)).
...
1. Audit residual rows in [rule-inventory-error.csv](../inventory/rule-inventory-error.csv) where `error_token` ∈ `"` `"` `'`.
