...
- Spec source of truth for accept/reject behaviour: `.scratch/rule-index/research/asca-rule-validity.md` — whole-rule form, per-field checklists, operators, ID failure patterns, CLI/`ParsedRules::try_from` validation (§4), and the lexer→parser→split_into_subrules→runtime map for a Python implementation (§5).
...
4. Validator unit tests should load the `asca_guess` rows (via pandas) and assert validate-success (or documented expected failure) against `DiachronicSeries` built from those guessed fields.

## Answer

**Gist:** `validate_asca(DiachronicSeries) -> True` lives in `src/conlanger/tools/asca_validator.py`; invalid rules raise `ASCAValidationError` with ASCA’s Syntax/Runtime Error text. Validation drives installed **asca 0.10.2** via `asca run` on a probe wordlist (hybrid Tier 1–4; same path as inventory). Fixture: 500 `asca_guess` rows (seed **20260802**) appended to `tests/fixtures/sound_change_rules.csv` — **370** validate ok, **130** documented expected failures (`asca_expect_ok=False`).
