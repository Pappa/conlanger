...
Index Diachronica places ejective `ʼ` **after** feature matrices and sets (`ts:[+long]ʼ`, `{t,ts}ʼ`). ASCA parses `ʼ` only as a **segment diacritic** (before `:`) or as the feature **`[+cg]`**. See `.scratch/rule-index/research/asca-ejective-notation.md`.
