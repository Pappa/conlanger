...
**Changelog:** run validation with **existing** `rule-inventory.csv` as baseline so `rule-inventory-changelog.csv` appends `ok` flips — but **do not** treat changelog as the sole metric.
...
- Prior scan: `.scratch/rule-index/research/scan_nested_sets.py`
