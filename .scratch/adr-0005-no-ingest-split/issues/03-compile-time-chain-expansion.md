...
Wire into `DiachronicSeries` / `SoundChangeRule` so `validate_asca` exercises each expanded step. Slot into compile order per [ASCA compile transform ordering](../cleaned-rule-corpus/research/asca-compile-transform-order.md).
...
- `DiachronicSeries` expands each corpus rule to one `SoundChangeRule` per chain step before compile; per-step rules run the full `compile_asca_rule_string` pipeline.
