...
Implement [Rule comment field on index rules](30-rule-comment-field-on-index-rules.md) in `IndexDiachronicaParser`:

1. Add optional **`comment`** to emitted index rule dicts; omit when empty.
...
5. Emit a short **analysis artifact** under `.scratch/cleaned-rule-index/` — e.g. top `comment` n-grams / qualifier phrases (`short only`, `unstressed`, …) for later parser iteration.
...
- [x] `comment` field on index rules when inline prose is stripped
- [x] Semicolon tails captured (~87 rules in current index have `;` in `raw`)
...
**Artifact:** [rule-comment-phrases.md](../rule-comment-phrases.md) — emitted by `uv run create_index`.
